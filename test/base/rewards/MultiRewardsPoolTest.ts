...
describe("multi reward pool tests", function () {
  ...
  before(async function () {
    ...
    await weth.mint(owner.address, parseUnits('100'));
    await weth.mint(user.address, FULL_REWARD);
    ...
    pool = await Deploy.deployContract(owner, 'MultiRewardsPoolMock', weth.address, owner.address, [rewardToken.address]) as MultiRewardsPoolMock;

    await weth.approve(pool.address, parseUnits('999999999'));
    await weth.connect(user).approve(pool.address, parseUnits('999999999'));
  ...
  it("notify checks", async function () {
    ...
    await expect(pool.connect(rewarder).notifyRewardAmount(weth.address, 10)).revertedWith('Wrong token for rewards');
