...
describe("gauge and bribe tests", function () {
  ...
  before(async function () {
    ...
    await weth.mint(owner.address, parseUnits('100000'));
  ...
  it("claim fees", async function () {
    ...
    await core.router.addLiquidityETH(
      mim.address,
      true,
      parseUnits('10000'),
      0,
    ...
    const pairAdr = await core.factory.getPair(mim.address, weth.address, true);
    ...
    expect(await weth.balanceOf(bribeAdr)).is.eq(0);
    expect(await mim.balanceOf(fees)).is.eq(0);
    expect(await weth.balanceOf(fees)).is.eq(0);

    await mim.approve(core.router.address, parseUnits('99999'));
    await core.router.swapExactTokensForTokens(
      parseUnits('1000'),
      0,
      [{from: mim.address, to: weth.address, stable: true}],
      owner.address,
      BigNumber.from('999999999999999999'),
    );
    await weth.approve(core.router.address, parseUnits('99999', 6));
    await core.router.swapExactTokensForTokens(
      parseUnits('1000', 6),
      0,
      [{to: mim.address, from: weth.address, stable: true}],
      owner.address,
      BigNumber.from('999999999999999999'),
    );

    expect(await mim.balanceOf(fees)).is.eq(parseUnits(EXPECTED_FEE));
    expect(await weth.balanceOf(fees)).is.eq(parseUnits(EXPECTED_FEE, 6));

    await gauge.claimFees();

    expect(await mim.balanceOf(fees)).is.below(2);
    expect(await weth.balanceOf(fees)).is.below(2);
    ...
    expect(await weth.balanceOf(bribe.address)).is.above(parseUnits(EXPECTED_FEE, 6).sub(2));

    expect(await bribe.left(mim.address)).is.above(100);
    expect(await bribe.left(weth.address)).is.above(100);
    ...
    expect(await weth.balanceOf(fees)).is.eq(EXPECTED_FEE2);

    await gauge.claimFees();

    expect(await mim.balanceOf(fees)).is.below(3);
    expect(await weth.balanceOf(fees)).is.below(3);
