...
describe("ve dist tests", function () {
  ...
  before(async function () {
    ...
    await weth.mint(owner2.address, parseUnits('10000'));

    const controller = await Deploy.deployContract(owner, 'Controller') as Controller;
    ve = await Deploy.deployVe(owner, weth.address, controller.address);
    veDist = await Deploy.deployVeDist(owner, ve.address);

    await weth.approve(ve.address, parseUnits('10000'))
    await weth.connect(owner2).approve(ve.address, parseUnits('10000'))
  ...
  it("multi checkpointToken with positive balance test", async function () {
    await veDist.setDepositor(helper.address);
    await weth.transfer(veDist.address, parseUnits('1'));
    await helper.multipleVeDistCheckpoints(veDist.address);
  });
  ...
  it("claim for early token test", async function () {
    const controller = await Deploy.deployContract(owner, 'Controller') as Controller;
    const ve1 = await Deploy.deployVe(owner, weth.address, controller.address);
    await weth.approve(ve1.address, parseUnits('10000'))
  ...
  it("claim complex test", async function () {
    ...
    console.log('ve balance before', formatUnits(await weth.balanceOf(ve.address)))
    console.log('vedist balance before', formatUnits(await weth.balanceOf(veDist.address)))

    await veDist.claim(1);
    await veDist.claim(2);
    await veDist.connect(owner2).claim(3);

    console.log('ve balance', formatUnits(await weth.balanceOf(ve.address)))
    console.log('vedist balance', formatUnits(await weth.balanceOf(veDist.address)))

    // expect(+formatUnits(await weth.balanceOf(veDist.address))).eq(0)
    ...
    for (let i = 0; i < 9; i++) {
      ...
      console.log('ve balance', i, formatUnits(await weth.balanceOf(ve.address)))
      console.log('vedist balance', i, formatUnits(await weth.balanceOf(veDist.address)))
    }
    ...
    console.log('ve balance final', formatUnits(await weth.balanceOf(ve.address)))
    console.log('vedist balance final', formatUnits(await weth.balanceOf(veDist.address)))
