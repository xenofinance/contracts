...
contract XenoPair is IERC20, IPair, Reentrancy {
  ...
  constructor() {
    ...
    IERC20(token0).approve(IFactory(factory).treasury(), type(uint256).max);
    IERC20(token1).approve(IFactory(factory).treasury(), type(uint256).max);
  ...
  function burn(address to) external lock override returns (uint amount0, uint amount1) {
    ...
    require(amount0 > 0 && amount1 > 0, 'XenoPair: INSUFFICIENT_LIQUIDITY_BURNED');
  ...
  function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external override lock {
    ...
    require(amount0Out > 0 || amount1Out > 0, 'XenoPair: INSUFFICIENT_OUTPUT_AMOUNT');
    (uint _reserve0, uint _reserve1) = (reserve0, reserve1);
    require(amount0Out < _reserve0 && amount1Out < _reserve1, 'XenoPair: INSUFFICIENT_LIQUIDITY');
    uint _balance0;
    uint _balance1;
    ...
    require(amount0In > 0 || amount1In > 0, 'XenoPair: INSUFFICIENT_INPUT_AMOUNT');
    {// scope for reserve{0,1}Adjusted, avoids stack too deep errors
      ...
      require(_k(_balance0, _balance1) >= _k(_reserve0, _reserve1), 'XenoPair: K');
    }
  ...
  function permit(
    ...
    require(recoveredAddress != address(0) && recoveredAddress == owner, 'XenoPair: INVALID_SIGNATURE');
  ...
  function transferFrom(address src, address dst, uint amount) external override returns (bool) {
    ...
    if (spender != src && spenderAllowance != type(uint).max) {
      require(spenderAllowance >= amount, "XenoPair: Insufficient allowance");
    unchecked {
  ...
  function _transferTokens(address src, address dst, uint amount) internal {
    ...
    require(srcBalance >= amount, "XenoPair: Transfer amount exceeds balance");
  unchecked {
